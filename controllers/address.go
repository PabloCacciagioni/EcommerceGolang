...
func AddAdress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		group := bson.D{{Key: "$group", Value: bson.D{primitive.E{Key: "_id", Value: "$address._id"}, {Key: "count", Value: bson.D{primitive.E{Key: "$sum", Value: 1}}}}}}
...
func DeleteAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		filter := bson.D{primitive.E{Key: "_id", Value: usert_id}}
		update := bson.D{{Key: "$set", Value: bson.D{{Key: "addresses", Value: addresses}}}}
		_, err = UserCollection.UpdateOne(ctx, filter, update)
		if err != nil {
